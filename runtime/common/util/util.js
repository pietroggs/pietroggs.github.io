var InternetExplorer = (navigator.userAgent.match(/Trident/i) ? true : false ) || ( navigator.userAgent.match(/MSIE/i) ? true : false );

function ieTridentVersion() {
	var match  = navigator.userAgent.match("Trident/([0-9]+).([0-9]+)")
	if ( match ) {
...
*/
function sendRequest(url,data,type, contentType) {
	if (isUndefined(data))
		data = null;
	if (isUndefined(type))
	...
	if (req) {
		...
		if (type=="POST" || typeof(contentType)=='string') {
			req.setRequestHeader("Content-Type",
					typeof(contentType)=='string' ? contentType : "application/x-www-form-urlencoded" );
		}
		req.send(data);
	}
	return req;
